...
export function flatRoof(props: FlatRoofProps): FlatRoofBomItem[] {
    ...
    if (orientation === "SOUTH") {
        // Excel NEEDED formulas for SOUTH layout (B2=rows, B3=columns)
        const needTable = columns * rows + columns; // I11: B3*B2+B3 (simplified, no checkbox)
        ...
        // EAST_WEST: simplified (Excel has ROUND(B2,0)*B3*2-IF(ISODD(B2),B3,0) etc.)
        const needBackplate = Math.round(rows) * columns * 2 - (rows % 2 === 1 ? columns : 0);
        const needLprofile = needBackplate;
        const needKoppel = columns + 1;
        const needEindrubber = needKoppel * 2;
        ...
        push("1SSP99AC030", needKoppel);
        push("1SSP99AC038", needEindrubber);
        push("1SSP99AC034", Math.max(0, needTable * 9 + needBackplate * 4 + needLprofile * 2));
        push("1HME46PL001", needTable * 6 + needBackplate * 4 + needLprofile * 4 + needKoppel * 2);
