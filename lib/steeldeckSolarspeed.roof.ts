...
export function steeldeckSolarspeedRoof(
    ...
    // SOUTH: eindklem cols×4, middenklem cols×(rows-1)×2. EAST_WEST: eindklem rows×4, middenklem rows×(cols-1)×2 (match Flat EAST_WEST).
    const needMiddenklem = orientation === "SOUTH" ? columns * (rows - 1) * 2 : rows * (columns - 1) * 2;
    const needEindklem = orientation === "SOUTH" ? columns * 2 * 2 : rows * 2 * 2;
